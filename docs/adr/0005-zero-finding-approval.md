...
Review Comment Mode が inline のときはサマリー付き PR レビュー 1 回の `event` を `APPROVE` にする。comment モード、および inline の `APPROVE` 失敗後の Issue コメントフォールバックでは、先に短い定型文の `APPROVE` を投稿してから Issue コメントにサマリーを載せる（リトライ時の Issue コメント重複を避けるため）。同一 Pull Request で Review Run が複数回 Zero-Finding になった場合も毎回 Approve する。指摘が 1 件以上ある Review Run では従来どおり `COMMENT` のみとし、過去の Approve を取り消す処理は行わない。
